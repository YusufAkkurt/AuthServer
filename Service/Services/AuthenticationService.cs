...
namespace AuthServer.Service.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public AuthenticationService(IOptions<List<Client>> options, ITokenService tokenService, UserManager<UserApp> userManager, IUnitOfWork unitOfWork, IRepository<UserRefreshToken> userRefreshTokenRepository)
        {
        ...
        public async Task<Response<TokenDto>> CreateTokenAsync(LoginDto loginDto)
        {
            ...
            var refreshToken = await _userRefreshTokenRepository.Where(row => row.UserId == user.Id).SingleOrDefaultAsync();
        ...
        {
            var client = _clients.SingleOrDefault(item => (item.Id == clientLoginDto.ClientId) && (item.Secret == clientLoginDto.ClientSecret));
            if (client == null)
        ...
        {
            var existRefreshToken = await _userRefreshTokenRepository.Where(row => row.Code == refreshToken).SingleOrDefaultAsync();
